...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader, desc=f"Epoch {epoch} - Train"), 0):
        ...
        # Make sure restored is a flat list of tensors
        if isinstance(restored, tuple):
            restored = list(restored)

        if isinstance(restored, list) and len(restored) > 0 and isinstance(restored[0], list):
            restored = restored[0]
        ...
        loss_char = sum(criterion_char(r, target) for r in restored)
        loss_edge = sum(criterion_edge(r, target) for r in restored)
